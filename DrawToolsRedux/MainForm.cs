...
using am = Messages.actionlib_msgs;
...
namespace DrawTools
{
	public partial class MainForm : Form
	{
        ...
        public static NodeHandle ROSNodeHandle;
		...
		public void SetStateOfControls()
		{
            ...
            trackBar1.Value = (int)(drawArea.Zoom * 10);
		}
        ...
        private void CommandGoal()
        {
            drawArea.ActiveTool = DrawArea.DrawToolType.SetGoal;
        }
		/// <summary>
		...
		private void MainForm_KeyDown(object sender, KeyEventArgs e)
		{
            ...
            //keyControl.Process_KeyDown(sender, e);
		...
		private void MainForm_KeyUp(object sender, KeyEventArgs e)
		{
            ...
            //keyControl.Process_KeyUp(sender, e);
		}
    ...
    private void cutToolStripMenuItem_Click(object sender, EventArgs e)
    {
      drawArea.CutObject();
        ...
        {
            Ros_CSharp.ROS.shutdown();
            Ros_CSharp.ROS.waitForShutdown();
            base.OnClosed(e);
        }

        private void toolStripButton2_Click(object sender, EventArgs e)
        {
            ROS.DrawRobot drawRobot = new ROS.DrawRobot(drawArea);
            drawRobot.SubscribeToFootprint("/move_base/global_costmap/footprint");

            AddNewObject(drawArea, drawRobot);
        }

        private void timer1_Tick(object sender, EventArgs e)
        {
            DateTime lastDataUpdateTime = drawArea.LastDataUpdateTime;
            if(lastDataUpdateTime.AddMilliseconds(1000)>DateTime.Now)
                drawArea.Invalidate();
        }

        private void toolStripButton3_Click(object sender, EventArgs e)
        {
            ROS.DrawLaserScan drawlaser = new ROS.DrawLaserScan(drawArea);
            drawlaser.SubscribeToLaserScan("/laser_pointcloud_map");

            AddNewObject(drawArea, drawlaser);
        }
        DrawTools.ROS.KeyControl keyControl;
        private void toolStripButton6_Click(object sender, EventArgs e)
        {
            if (keyControl == null)
            {
                keyControl = new ROS.KeyControl(drawArea);
                this.KeyDown += keyControl.Process_KeyDown;
                this.KeyUp += keyControl.Process_KeyUp;
            }
            if(!toolStripButton6.Checked)
            {
                keyControl.StartControl();
                toolStripButton6.Checked = true;
            }
            else
            {
                keyControl.ExitControl();
        ...
        {
            ROS.DrawPath drawPath = new ROS.DrawPath(drawArea);
            drawPath.SubscribeToPath("/move_base/NavfnROS/plan");

            AddNewObject(drawArea, drawPath);
        }
        ...
        Publisher<am.GoalID> goalCancelPub;
        ...
        DrawTools.ROS.PassiveControl passiveControl;
        private void toolStripButton7_Click(object sender, EventArgs e)
        {
            if(passiveControl==null)
            {
                passiveControl = new ROS.PassiveControl(drawArea);
            }
            if (!toolStripButton7.Checked)
            {
                passiveControl.StartControl();
                toolStripButton7.Checked = true;
            }
            else
            {
                passiveControl.ExitControl();
        ...
        {
            drawArea.Zoom = trackBar1.Value / 10f;
            if (drawArea.Zoom < .1f)
                drawArea.Zoom = .1f;
            if (drawArea.Zoom > 10)
                drawArea.Zoom = 10f;
            ...
            if (!toolStripButton11.Checked)
            {
                using (System.Windows.Forms.OpenFileDialog ofd = new System.Windows.Forms.OpenFileDialog())
                {
                    ...
                    if (ofd.ShowDialog() == DialogResult.OK)
                    {
                        ...
                        if (mapServer != null)
                            return;
                        mapServer = new MapServer.Map_server(ROSNodeHandle);
                        mapServer.LoadConfig(ofd.FileName);
                        mapServer.Run();
        ...
        private void toolStripButton12_Click(object sender, EventArgs e)
        {
            using (System.Windows.Forms.SaveFileDialog ofd = new System.Windows.Forms.SaveFileDialog())
            {
                ...
                if (ofd.ShowDialog() == DialogResult.OK)
                {
                    ...
                    drawMap.SaveMap(ofd.FileName);
                   // map_saver = new MapServer.Map_saver(ROSNodeHandle);
                    //map_saver.SaveMap(System.IO.Path.GetFileName(ofd.FileName), System.IO.Path.GetFullPath(ofd.FileName));
        ...
        {
            ROS.DrawRobotPosition drawRobot = new ROS.DrawRobotPosition(drawArea);
            drawRobot.SubscribeToPostion("/robot_baselink");
